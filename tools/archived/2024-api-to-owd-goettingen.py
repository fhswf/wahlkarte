...
class Wahlparameter:
    data: Dict[str, Any]
    termin: Dict[str, Any]
    wahlBehoerdeGS: str = "03159016"
    wahlBehoerdeName: str = "Stadt Göttingen"
...
class Wahlgebietseinteilungen:
    ...
    wahlName: str
    wahlBehoerdeGS: str = "03159016"
    wahlLeiterGS: str = "03159016"
    wahlLeiterName: str = "Stadt Göttingen"
    kandGebNr: str = ""
    kandGebBez: str = ""
    stimmGebNr: str = ""
    stimmGebBez: str = ""
    ...
    def writeOWDcsv(self) -> None:
        with open(f"./{self.wahlBehoerdeGS}_{self.datum}_{self.wahlName}_Wahlgebietseinteilungen_V0-3_{(self.uebersicht_data.get('file_timestamp') or self.uebersicht_data.get('zeitstempel')).replace(':', '')}.csv", "w", newline="", encoding="utf-8") as csvf:
            ...
            def _name_to_id(name) -> str:
                ...
                assert name
                return ''.join(_ for _ in name.replace("Stadt Göttingen - ", "").split(" ")[0] if _.isdigit()) or name
            ...
            for bezirk_id_intern, bezirk_data in self.bezirke_data.items():
                ...
                csvw.writerow((
                    ...
                    bezirk_id, bezirk_name, briefwahlbezirk_id, "B" if "BW-" in bezirk_name else "W", "",
...
base = f"https://votemanager.kdo.de/20240609/03159016/"
